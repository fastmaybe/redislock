 ...
 */
public class ReentrantZKLock extends AbstractLockImpl {


    private ZkClient zkClient = new ZkClient("127.0.0.1:2181");

    private final String PATH = "/rentpath";
...
//                    System.err.println(beforePath);
            ...
            count = nextS;
            System.out.println(Thread.currentThread().getName()+" 获取锁次数"+count);
            return true;
        }
...
//            System.err.println(Thread.currentThread().getName()+"=== 阻塞====；监听"+beforePath);

            try {
                latch.await();
//                System.err.println(Thread.currentThread().getName()+"=== 继续运行====；监听"+beforePath);
